...
class CreateDataEntryViewPage extends Component {
  ...
  getSessionInfoToDisplay = (session) => {
    ...
    const newSessionData = sessionJson.reduce( (container, obj)  => {
      return Object.assign(container, obj);
    }, {'Session ID': session.session_id});
    newSessionData['Date and Time'] = moment(new Date(session.date_created * 1000)).format('YYYY/MM/DD HH:mm');
  ...
  render() {
    ...
    return (
      <div>
        {this.state.sessions ? <SelectSession
