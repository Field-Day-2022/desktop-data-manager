...
export default withStyles(styles)(
  class NewSessionEntryForm extends Component {
    ...
    handleSubmitSession = async () => {
      ...
      if(isNaN(sessionId)) {
        this.setState({sessionError: "The date and time fields are required and must match the specified format."});
        return;
      }else {
          const sessionData = this.state.sessionData;
          const newSession = {
          ...
          if(!idExists) {
            ...
            this.setState({sessionError: "That date and time has already been used for a session. Please modify the date/time to make it unique."});
          }
      }
    ...
    render() {
      ...
      const showSessionForm = this.state.selectedForm !== '' && this.state.fields !== undefined;
      ...
      return (
        <Paper className={classes.container}>
          ...
          {(this.state.sessionError.length > 0) && <p className={classes.errors}>{this.state.sessionError}</p>}
          {textFields !== undefined
              &&
              <div>
                <form>
                  <h5>Enter the data and time of the session (Note: the date and time entered must be unique and not used for any other session)</h5>
                  <TextField
                    id="sessionDate"
                ...
                <Button onClick={this.handleSubmitSession} variant='contained' color='primary'>Submit</Button>
              </div>}
