...
export default withStyles(styles)(
  class NewDataEntryForm extends Component {
    ...
    handleSubmitEntry = async () => {
      ...
      if (isNaN(entryId)) {
        this.setState({ entryError: "The date and time fields are required and must match the specified format." });
        return;
      } else {
        const entryData = this.state.entryData;
        const newEntry = {
        ...
        const idUsed = this.state.entries.some(e => { return e.entry_id === entryId;});
        if (!idUsed) {
          ...
          this.setState({ entryError: "That date and time has already been used for an entry in this session. Please modify the date/time to make it unique." });
        }
      }
      ...
      if (this.state.entryAdded) {
        ...
        return (
          <Paper className={classes.container}>
            ...
            <Button onClick={this.resetForm} variant='contained' color='primary'>Add Another Entry</Button>
          </Paper>
        );
      }

      const options = dataForms.map(form => {
        return <MenuItem key={form.form_id} value={form.form_id}>{form.form_name}</MenuItem>;
      });
      ...
      return (
        <Paper className={classes.container}>
          ...
          <FormControl className={classes.formControl}>
            ...
            <FormHelperText>Select the data form to use for the new entry</FormHelperText>
          </FormControl>
          {showForm && <h4>Enter Data Entry Information</h4>}
          {(this.state.entryError.length > 0) && <p className={classes.errors}>{this.state.entryError}</p>}
          {textFields !== undefined
            &&
            <div>
              <form>
                <h5>Use the provided data/time, or enter a different data and time for this entry (Note: the date and time entered must be unique and not used for any other entry in this session)</h5>
                <TextField
                  id="sessionDate"
              ...
              <Button onClick={this.handleSubmitEntry} variant='contained' color='primary'>Submit</Button>
            </div>}
