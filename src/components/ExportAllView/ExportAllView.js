...
class ExportAllView extends Component {
    ...
    getData = async () => {
        ...
        for (let x = 0; x < sortedEntries.length; x++) {
            sortedEntries[x]['session_json'] = sortedSession.find(
                (s) => s.session_id === sortedEntries[x].session_id
            ).session_json;
        }
    ...
    getSelectedForms = () => {
        const allForms =
            this.state.formType === DataFormType ? this.state.dataForms : this.state.sessionForms;
    ...
    getEntries = () => {
        const allRows =
            this.state.formType === DataFormType ? this.state.entryRows : this.state.sessionRows;
    ...
    render() {
        ...
        return (
            <Paper className={classes.container}>
                <h3 className={classes.detailHeader}>
                    Export Data From Multiple Forms Into One CSV File
                </h3>
                <p>
                    The exported file will contain the column headers and values in each of the
                    selected forms.
                </p>
                ...
                <FormControl component="fieldset" className={classes.formControl}>
                    <FormLabel component="legend">
                        Please select the forms to include in the file.
                    </FormLabel>
