  ...
  class SelectSession extends Component {
    constructor(props) {
      ...
      if (this.props.sessions.length) {
        ...
        this.state.selectedYear = moment(new Date(this.state.selectedSession * 1000)).format('YYYY');
      }
    ...
    render() {
      ...
      const sessionsByYear = (this.state.sessionsByYear === undefined)
      ...
      const sessionOptions = sessionsByYear[this.state.selectedYear].map(session => {
        ...
        return <MenuItem key={session.session_id} value={session.session_id}>{title}</MenuItem>;
      });
      return (
        <Paper className={classes.container}>
          ...
          <div>
            <Button onClick={this.submitSessionSelection} className={classes.spacing} variant='contained' color='primary'>Select Session</Button>
          </div>
        </Paper>
      );
    }
