    ...
    getRows = async () => {
        ...
        try {
            ...
            process.env.REACT_APP_BATEMAN_BUILD === 'true' &&
                dataForms.forEach((f) => {
                    const hasSpeciesCode = JSON.parse(f.template_json).fields.some((f) => {
                    ...
                    if (hasSpeciesCode) {
                        const speciesAnswerSet =
                            hasSpeciesCode &&
                            JSON.parse(
                                getAnswerSet(getProjectName(project_id) + f.form_name + 'Species')
                                    .answers
                            );
                        formSpp[f.form_id] = {
                            hasSpeciesCode: true,
            ...
            const entries = data
                ...
                .map((entry) => {
                    ...
                    row['Date/Time'] = moment(new Date(entry.date_created * 1000)).format(
                        'YYYY/MM/DD HH:mm'
                    );
                    ...
                    if (process.env.REACT_APP_BATEMAN_BUILD === 'true') {
                        ...
                        if (formSpecies.hasSpeciesCode) {
                            const species = formSpecies.speciesAnswerSet.find((s) => {
                ...
                <Dialog fullScreen open={open} onClose={onClose} TransitionComponent={Transition}>
                    ...
                    <div className={classes.paddingContainer}>
                        ...
                        {Object.values(entryTableData).map((data) => {
                            ...
                            if (hasSpeciesCode) {
                                const sppIndex = fields.findIndex((f) => {
                                    return f.prompt === 'Species Code';
                                });
                                fields.splice(
                                    sppIndex + 1,
