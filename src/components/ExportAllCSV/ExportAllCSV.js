...
class ExportAllCSV extends Component {
  ...
  getRows = (headers) => {
    const { sessionForms, dataForms, selectedRows, answerSets, projects, project_id, allSessions } = this.props;
  console.log("all sessions ", allSessions)
    ...
    (process.env.REACT_APP_BATEMAN_BUILD === 'true') &&
      forms.forEach(f => {
        ...
        const hasSpeciesCode = (!f.is_session_form)
          ? template.fields.filter(f => f.type !== 'HIST_BUTTON').some(f => { return (f.prompt === "Species Code"); })
    ...
    const rows = selectedRows.map(entry => {
      ...
      if (entry.hasOwnProperty('entry_id')) {
        ...
        row['Date/Time'] = moment(new Date(entry.session_id * 1000)).format('YYYY/MM/DD HH:mm:ss');
      }
      ...
      if (process.env.REACT_APP_BATEMAN_BUILD === 'true') {
        ...
        if (formSpecies.hasSpeciesCode) {
          const species = (formSpecies.speciesAnswerSet) && formSpecies.speciesAnswerSet.find(s => { return (s.primary === row['Species Code']); });
  ...
  getHeaders = (forms) => {
    ...
    forms.forEach(form => {
      ...
      if (!form.is_session_form) {
        ...
        const yearIndex = fields.findIndex(f => { return (f.prompt === 'Year'); });
