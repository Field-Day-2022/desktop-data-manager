...
class DataFormViewPage extends Component {
        ...
        if (force || !form || form.form_id !== newForm.form_id) {
            try {
                ...
                const hasSpeciesCode =
                    ...
                    newForm.template_json.fields.some((f) => {
                        return f.prompt === 'Species Code';
                    });

                const speciesAnswerSet =
                    hasSpeciesCode &&
                    getAnswerSet(getProjectName(project_id) + newForm.form_name + 'Species')
                        .answers;
                ...
                const rows = entries
                    ...
                    .map((entry) => {
                        ...
                        row['Year'] = moment(new Date(session['date_created'] * 1000)).format(
                            'YYYY'
                        );
                        row['Session ID'] = entry.session_id;

                        if (process.env.REACT_APP_BATEMAN_BUILD === 'true') {
                            const site = session.session_json.find(
                                (f) => Object.keys(f)[0] === 'Site'
                            );
                            if (site) row['Site'] = site['Site'];
                            const array = session.session_json.find(
                                (f) => Object.keys(f)[0] === 'Array'
                            );
                            if (array) row['Array'] = array['Array'];
                            if (hasSpeciesCode) {
                                const species = speciesAnswerSet.find((s) => {
    ...
    render() {
        ...
        if (form) {
            ...
            const hasSpeciesCode =
                process.env.REACT_APP_BATEMAN_BUILD === 'true' &&
                form.template_json.fields.some((f) => {
                    return f.prompt === 'Species Code';
                });
            process.env.REACT_APP_BATEMAN_BUILD === 'true' &&
