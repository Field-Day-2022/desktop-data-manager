...
class CreateSessionViewPage extends Component {
  ...
  render() {
    ...
    const selectedSessionForm = this.state.sessionAdded ? sessionForms.find(sf => {
        return sf.form_id === this.state.newSession.form_id;
      }) : undefined;

    const newSessionData = this.state.sessionAdded ? this.state.newSession.session_json.reduce( (container, obj)  => {
      return Object.assign(container, obj);
    }, {'Session ID': this.state.newSession.session_id}) : undefined;

    if(newSessionData) {
      newSessionData['Date and Time'] = moment(new Date(newSessionData['Session ID'] * 1000)).format('YYYY/MM/DD HH:mm');
    ...
    return (
      <div>
        ...
        {this.state.sessionAdded && <SessionDetailCard title={selectedSessionForm.form_name} row={newSessionData} />}
